...
public class ProductoPedidoController {
    ...
    public ResponseEntity<ProductoPedidoDTO> createProductoPedido(@RequestBody ProductoPedidoDTO productoPedidoDTO) {
        ProductoPedidoDTO savedProductoPedido = productoPedidoService.save(productoPedidoDTO);
        return ResponseEntity.status(HttpStatus.CREATED).body(savedProductoPedido);
    }
